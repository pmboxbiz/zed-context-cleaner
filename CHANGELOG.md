...
- **Fix GPT->Claude switch actually works now** — root cause: `RedactedThinking` in Zed JSON is stored as `{"RedactedThinking": "base64string"}` (string value), not `{"RedactedThinking": {"data": ...}}` (object). Changed `RedactedThinkingBlock` to `Value` so it deserializes correctly. Previously the blocks were silently ignored during cleanup.
- **CLI `--fix-thinking` flag** added to `clean` command for fixing thinking blocks from command line
- Thinking block byte count now correctly includes RedactedThinking in statistics

## [0.3.2] - 2025-05-04

### Added

- **Fix GPT->Claude switch** — new checkbox removes `RedactedThinking` blocks from ALL messages (including protected). Fixes Anthropic API error `Invalid data in redacted_thinking block` when switching from a thinking model (GPT 5.5, Claude Opus with extended thinking) to a non-thinking model (Sonnet 4.6).
